...
- working with roles.
- using roles in Thymeleaf views.

#### Project configuration

Add Gradle dependencies for Spring Security in `build.gradle`:
```groovy
...
generated with annotation @RequiredArgsConstructor.
...
#### Exercise 10
Add in class SecurityJpaConfig a dependency to JpaUserDetailsService.
...
#### Exercise 11
Run the application with the mysql profile and verify that Spring Security's auto-configured default login form appears at http://localhost:8080/products.
At this point the `SecurityFilterChain` has not been configured yet, so Spring Boot automatically protects all endpoints with its built-in login page.
...
#### Exercise 16
Add Thymeleaf sec:authorize on product/form url in templates/main.html.
