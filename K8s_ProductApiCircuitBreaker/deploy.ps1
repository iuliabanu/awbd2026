...
kubectl apply -f secrets-cert-manager.yaml --timeout=120s
...
kubectl apply -f springbootapp/discount-deployment.yaml
kubectl apply -f springbootapp/discount-service.yaml
...
kubectl rollout status deployment/keycloak -n auth --timeout=320s

# --- Import the demo realm -----------------------------------
...
kubectl rollout status deployment/gateway -n app --timeout=120s
